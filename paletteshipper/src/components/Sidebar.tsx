...
const Sidebar = () => {
  ...
  const getIconColor = (options: { disabled?: boolean } = {}) => {
    if (options.disabled) return 'rgba(255, 255, 255, 0.6)';
    return '#ffffff';
  };
  ...
  const handleLogout = async () => {
    ...
    if (galleryUrl) {
      const target = `${galleryUrl}/auth?logout=1&return=${encodeURIComponent(returnTo)}`;
        ...
        <div className="bottom-menu">
          ...
          >
            {(isLoggingOut || authLoading) ? (
              ...
              <LogoutOutlinedIcon sx={{ color: '#ffffff' }} className="icon-24" />
            )}
            <div className="sidebar-btn-text">
