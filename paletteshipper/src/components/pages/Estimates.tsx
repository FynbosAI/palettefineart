...
import { Button, CircularProgress, Alert, Checkbox, FormControlLabel } from '@mui/material';
import SecurityIcon from '@mui/icons-material/Security';
import AcUnitIcon from '@mui/icons-material/AcUnit';
import LocalShippingIcon from '@mui/icons-material/LocalShipping';
import MessageOutlinedIcon from '@mui/icons-material/MessageOutlined';
import { useNavigate, useLocation } from 'react-router-dom';
import { useQuotes, useAuth, useLoadingState, useBids } from '../../hooks/useStoreSelectors';
...
const Estimates = () => {
  ...
  const handleWithdraw = async (quoteId: string) => {
    ...
    const confirmed = window.confirm('Withdraw your bid? This cannot be undone.');
    if (!confirmed) return;
    try {
      ...
      alert('Failed to withdraw bid: ' + (err?.message || String(err)));
    } finally {
    ...
    return combinedQuotes
      ...
      .map(quote => {
        ...
        return ({
          ...
          galleryLogoUrl: quote.owner_org?.img_url,
                  ...
                  <div className="thumb" style={{ 
                    ...
                    {request.galleryLogoUrl ? (
                      ...
                      <div style={{
                        ...
                        {request.gallery?.[0]?.toUpperCase() || 'G'}
                      </div>
                    )}
                  </div>
                  <div className="title">
                      ...
                      : isDeadlineExpired
                        ...
                        : deadlineUrgency === 'critical'
                          ...
                          : deadlineUrgency === 'warning'
                            ? '#E9932D'
                            : '#8412ff';
                    const buttonLabel = myBid?.status === 'pending' ? 'Re-bid' : 'View Details';
                    const bidRowContent = (() => {
                      const hasBid = hasSubmittedBid(request.id);

                      if (myBid && myBid.status === 'needs_confirmation') {
                        return {
                          label: 'Bid requires confirmation',
                          labelColor: '#e11d48',
                          fontWeight: 600,
                      ...
                      if (hasBid && myBid) {
                        return {
                          label: `Your bid: ${formatCurrency(myBid.amount)}`,
                          labelColor: '#22c55e',
                          fontWeight: 600,
                          fontSize: '16px',
                          withdrawable: canWithdrawBid(myBid),
                      ...
                      if (myBid && myBid.is_draft) {
                        return {
                          label: `Draft bid: ${formatCurrency(myBid.amount)}`,
                          labelColor: '#f59e0b',
                          fontWeight: 500,
                          fontSize: '12px',
                          withdrawable: false,
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                              {manualClose || isDeadlineExpired ? (
                                ...
                                >
                                  {manualClose ? request.deadlineState?.label || 'Manual close' : 'Bidding closed'}
                                </span>
                              ) : (
                                <CountdownClock
