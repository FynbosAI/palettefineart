...
const ViewBidsPage = () => {
  ...
  const { subscribeToQuote } = useRealtime();
  ...
  // Subscribe to real-time bid updates for this specific quote
  useEffect(() => {
    if (quoteId) {
      logger.debug('ViewBidsPage', 'Subscribing to bid updates for quote');
      subscribeToQuote(quoteId);
    }
  }, [quoteId, subscribeToQuote]);
  ...
  const transformQuoteData = (quote: QuoteWithDetails): TransformedQuoteInfo => {
    ...
    // Get actual items count from quote artworks or shipment artworks
    const itemsCount = (quote.quote_artworks?.[0]?.count || 0) || 1;
  ...
  const transformBidData = (bid: BidWithPartner): TransformedBid => {
    ...
    const partnerName = bid.logistics_partner?.name || 'Unknown Partner';
    ...
    const abbreviation = branchLabel
      ? branchLabel
          ...
          .join('')
          .slice(0, 3) || '?'
      : bid.logistics_partner?.abbreviation || '?';
  ...
  const handleAcceptBid = async (bidId: string) => {
    ...
    if (!bid.branchOrgId) {
      const message = 'This bid is missing branch information and cannot be accepted yet.';
    ...
    try {
      ...
      if (result.error) {
        ...
        alert('Bid accepted successfully! A shipment has been created.');
      ...
      <div className="main-wrap">
        <div className="main-panel">
          ...
          <div className="main-content" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '400px' }}>
            <div>Loading quote and bids...</div>
          </div>
        </div>
      </div>
    );
              ...
              <div style={{ 
                ...
                <div>
                  <div style={{ fontSize: '14px', color: 'rgba(23, 8, 73, 0.7)' }}>
                    {quoteInfo.bidsReceived} bids received
                  </div>
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Button
                          ...
                          onClick={() => handleMessageBidder(bid.id, bid.shipper.name)}
                          sx={{
