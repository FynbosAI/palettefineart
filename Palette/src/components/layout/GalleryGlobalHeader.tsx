...
const GalleryGlobalHeader = () => {
  const location = useLocation();
  const excludedPrefixes = ['/estimates/new'];
  const shouldRender = !excludedPrefixes.some((prefix) => location.pathname.startsWith(prefix));
  const portalHost = useHeaderPortal(shouldRender);
...
export default GalleryGlobalHeader;
