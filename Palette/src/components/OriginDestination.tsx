...
const BUSINESS_DAY_DEFAULT_OFFSET = 3;
...
export const computeDefaultDeadline = (baseDate: Date = new Date()) => {
  ...
  const defaultDate = addBusinessDays(now, BUSINESS_DAY_DEFAULT_OFFSET);
...
const OriginDestination: React.FC<OriginDestinationProps> = ({ 
    ...
    const getDeadlineValidation = useCallback((deadlineIso: string | null) => {
        ...
        if (noAvailableDeadlineWindow) {
            return {
                valid: false,
                error: 'Arrival date is too soon to auto-close bidding. Adjust the arrival date or disable automatic closing.',
            } as const;
        }
        ...
        if (!deadlineIso) {
            return {
                valid: false,
                error: 'Select a bidding deadline to auto-close quotes.',
            } as const;
        }
    ...
    const biddingContent = (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div>
                <h2 style={{ marginBottom: '4px' }}>Bidding Deadline</h2>
                <p style={{ fontSize: '12px', color: 'rgba(23, 8, 73, 0.7)' }}>
                    Shippers lose bidding access once the deadline passes. The deadline must be set at least two full days before the arrival date.
                </p>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {deadlineError ? (
                    ...
                    <span style={{ fontSize: '12px', color: 'rgba(23, 8, 73, 0.6)' }}>
                        Recommended deadline: {recommendedDeadlineText}{arrivalStartDate ? ' (five days before arrival).' : ' (three business days from today).'}
                    </span>
                )}
                <Button
                    variant="text"
            ...
            {!autoCloseBidding && (
                <span style={{ fontSize: '12px', color: 'rgba(23, 8, 73, 0.6)' }}>
                    Bidding will stay open until you manually close the quote.
                </span>
            )} */}
        </div>
    );
