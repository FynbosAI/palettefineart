...
const ArtworksList: React.FC = () => {
  ...
  }, [selectedTransport, selectedInsurance, selectedServices.length, selectedPartners.length, isRequiredByDateValid]);
  ...
  const handlePartnerToggle = (partnerName: string) => {
  ...
  const handleSubmitShipment = async () => {
    ...
    if (!selectedTransport || !selectedInsurance || selectedServices.length === 0 || selectedPartners.length === 0) {
      alert('Please select transport method, insurance, services, and partners before submitting.');
      return;
    }
    ...
    >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 2, width: '100%', maxWidth: '800px', mx: 'auto' }}>
            ...
            <Paper elevation={0} sx={{ 
              ...
              {/* ICEFAT Certified Partners */}
              <Box>
                <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>ICEFAT Certified Partners</Typography>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                  {partners.map(partner => (
                    <Button
                      key={partner.id}
                      variant={selectedPartners.includes(partner.name) ? 'contained' : 'outlined'}
                      size="small"
                      onClick={() => handlePartnerToggle(partner.name)}
                      sx={{ 
                        fontSize: '0.7rem',
                        py: 0.3,
                        px: 1,
