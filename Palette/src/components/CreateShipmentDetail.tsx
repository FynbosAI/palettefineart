...
const buildShipperDisplayName = (record: LogisticsPartnerBranchRecord): string => {
  const companyName = record.branchNetwork?.companyName ?? record.company?.name ?? record.partner.name ?? 'Unknown partner';
...
const CreateShipmentDetail: React.FC = () => {
  ...
  const upsertQuote = async (status: 'draft' | 'active') => {
    ...
    if (shipmentForm.autoCloseBidding && status === 'active' && !isDeadlineValid) {
      alert('Set a valid bidding deadline before submitting the request.');
      return;
    }
    ...
    try {
      ...
      const originLoc = await LocationService.findOrCreateLocation(shipmentForm.origin, shipmentForm.origin);
      const destinationLoc = await LocationService.findOrCreateLocation(shipmentForm.destination, shipmentForm.destination);
      ...
      if (quoteId) {
        ...
        const quoteArtworks = shipmentForm.artworks.map(artwork => ({
          ...
          weight: artwork.weight?.toString() || null,
          ...
          <div style={{ 
            ...
            <div className="detail-card">
              ...
              {!loadingShippers && branchFilterMeta.branchFilterApplied && availableShippers.length === 0 && (
                <Alert severity="info" sx={{ mb: 2 }}>
                  No eligible shipper branches currently have active members. Ask your shipper partners to sign in so you can notify them.
                </Alert>
              )}
              ...
              {loadingShippers && (
                <div style={{ padding: '16px', textAlign: 'center' }}>
                  <span>Loading shipping partners...</span>
                </div>
              )}
            </div>
