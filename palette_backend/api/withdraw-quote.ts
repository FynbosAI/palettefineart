...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { p_quote_id, p_reason } = (req.body || {}) as { p_quote_id?: string; p_reason?: string }
    if (!p_quote_id) {
      res.status(400).json({ error: 'Missing p_quote_id' })
      return
    }
    ...
    const { error } = await supabaseAsUser.rpc('withdraw_quote', {
      p_quote_id,
      p_reason,
    })
