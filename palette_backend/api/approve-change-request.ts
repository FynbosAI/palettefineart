...
export default async function handler(req: VercelRequest, res: VercelResponse) {
	...
	try {
		const { p_change_request_id, p_response_notes, p_branch_org_id } = (req.body || {}) as {
			p_change_request_id?: string
			p_response_notes?: string
			p_branch_org_id?: string
		}
		if (!p_change_request_id) {
			res.status(400).json({ error: 'Missing p_change_request_id' })
			return
		}
		if (!p_branch_org_id) {
		...
		const { data, error } = await supabaseAsUser.rpc('approve_change_request', {
			p_change_request_id,
			p_response_notes,
			p_branch_org_id,
		})
