...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { p_shipment_id, p_new_deadline } = (req.body || {}) as { p_shipment_id?: string; p_new_deadline?: string }
    if (!p_shipment_id || !p_new_deadline) {
      res.status(400).json({ error: 'Missing p_shipment_id or p_new_deadline' })
      return
    }
    ...
    const { data, error } = await supabaseAsUser.rpc('reopen_shipment_for_bidding', {
      p_shipment_id,
      p_new_deadline,
    })
